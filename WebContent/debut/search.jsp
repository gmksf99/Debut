...
<%
    ...
    try{
    		...
    		watches = dao.selectBoth(conn, keyField, keyWord, searchPrice);
...
<!-- page_center영역 -->
  <div id="page_center" style="height:400px;">
    <div id="orderbyList">
    <form name="searchKeyword" method ="post">
    <select name="keyField">
      <option value = "0">--선택--</option>
      <option value="watchName">제품명</option>
      <option value="brand">브랜드</option>
      <option value = "memo">상세정보</option>
    </select> 
      <input name="keyWord" type="text" />
    <select name="searchPrice">
      <option value = "0">전체가격대</option>
      <option value="1">10만원 미만</option>
      <option value="2">10만원대</option>
      <option value="3">20만원대</option>
      <option value="4">30만원대 이상</option>
    </select> 
	  	...
	  	<c:forEach var = "watch" items = "${list }">
	  		<div class="item">
      			<table width="200px" height="300px" cellspacing="0" cellpadding="1">
  					...
  					<tr>
  					  <td class = "watchName">
  					  <strong>${watch.watchName }</strong></td>
  					</tr>
     			   <tr>
  					  <td class = "price">
  					  ￦ ${watch.price }
...
<c:set var = "height" value="${(fn:length(list)/3)*300+140 }"/>
</c:if>
